...
int main(){
    ...
    // - Get rid of commnds.{h,c}

    RecordsManager * manager = rm_load_records("helper.txt");

    if(manager == NULL){
        printf("Error loading records from 'helper.txt': %s\n", strerror(errno));
        return 1;
    }

    printf("Loaded %zu records from 'helper.txt'\n", rm_records_number(manager));
    RM_FOR_EACH_RECORD(manager, record, {
...
//     puts("    * get <name>            - moves to the path of <name> if such exists");
